...
public class GiphyService {
    ...
    public String getGiphyUrl(String searchTerm) {
        Gson gson =
                new Gson();
        String giphy =
                GIPHY_BASE + searchTerm + KEY;

        try {
            URI uri = UriComponentsBuilder.
                    fromUriString(giphy).build().encode().toUri();
            MultiValueMap<String, String> mvm =
                    new LinkedMultiValueMap<>();
            ResponseEntity<String> res =
                    rest.exchange(uri, HttpMethod.GET, new HttpEntity(mvm, null), String.class);
            JsonObject json = gson.fromJson(res.getBody(), JsonObject.
                    class);
            ...
            JsonObject images = ((JsonObject) data.get(0)).getAsJsonObject(
                    "images");
            JsonObject original = images.getAsJsonObject(
                    "original");
